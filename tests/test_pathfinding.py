...
from snake.configs.game import (  # Using actual game dimensions for now
    BLOCK_SIZE,
)
...
def configure_mock_pygame(mock_pygame_obj):
    ...
    mock_pygame_obj.display.set_caption.return_value = (
        MagicMock()
    )  # Though not strictly necessary for pathfinding logic
    mock_pygame_obj.time.Clock.return_value = (
        MagicMock()
    )  # Though not strictly necessary for pathfinding logic
...
class TestPathfindingAlgorithms(unittest.TestCase):
    ...
    def run_test_simple_path(
        self, algorithm_class, algorithm_name, current_mock_pygame
    ):
        configure_mock_pygame(
            current_mock_pygame
        )  # Configure mock before algorithm instantiation
        ...
        self.assertTrue(
            len(algo.path) > 0,
            f"{algorithm_name}: Path should not be empty for a simple case. Path: {algo.path}",
        )
        self.assertEqual(
            len(algo.path),
            2,
            f"{algorithm_name}: Path length should be 2 for this simple case. Path: {algo.path}",
        )
        if algo.path and len(algo.path) == 2:
            ...
            self.assertEqual(
                algo.path[0],
                first_step_expected,
                f"{algorithm_name}: First step is incorrect.",
            )
            self.assertEqual(
                algo.path[1], algo.food, f"{algorithm_name}: Last step should be food."
            )
        for p_idx, p_obj in enumerate(algo.path):  # Check all items are Point objects
            self.assertIsInstance(
                p_obj,
                Point,
                f"{algorithm_name}: Item at index {p_idx} in path is not a Point object.",
            )

    def run_test_no_path_blocked(
        self, algorithm_class, algorithm_name, current_mock_pygame
    ):
        configure_mock_pygame(current_mock_pygame)
        algo = algorithm_class(
            game_has_obstacles=True
        )  # game_has_obstacles can be True
        ...
        algo.obstacles = [
            ...
            ),  # X3: Blocks H -> (0,BS) -> X3 (if X2 not there)
            ...
            Point(
                BLOCK_SIZE * 3, 0
            ),  # X5: Blocks approach from right of food (ensure in bounds)
        ]
        ...
        self.assertEqual(
            len(algo.path),
            0,
            f"{algorithm_name}: Path should be empty when food is blocked. Path found: {algo.path}",
        )

    def run_test_path_with_obstacles(
        self, algorithm_class, algorithm_name, current_mock_pygame
    ):
        configure_mock_pygame(current_mock_pygame)
        ...
        self.assertTrue(
            len(algo.path) > 0,
            f"{algorithm_name}: Path should be found around obstacle. Path: {algo.path}",
        )
        if algo.path:
            self.assertEqual(
                algo.path[-1], algo.food, f"{algorithm_name}: Path should lead to food."
            )
            for p_obstacle in algo.obstacles:
                self.assertNotIn(
                    p_obstacle,
                    algo.path,
                    f"{algorithm_name}: Path should not go through obstacles.",
                )
        ...
        for p_idx, p_step in enumerate(algo.path):
            self.assertIsInstance(
                p_step,
                Point,
                f"{algorithm_name}: Path item at index {p_idx} is not a Point.",
            )
            dx = abs(p_step.x - last_point.x)
            dy = abs(p_step.y - last_point.y)
            is_valid_step = (dx == BLOCK_SIZE and dy == 0) or (
                dx == 0 and dy == BLOCK_SIZE
            )
            self.assertTrue(
                is_valid_step,
    ...
    def run_test_food_adjacent(
        ...
        self.assertEqual(
            len(algo.path),
            1,
            f"{algorithm_name}: Path should be 1 step long. Path: {algo.path}",
        )
        if algo.path:  # Check if path is not empty before accessing its elements
            self.assertEqual(
                algo.path[0],
                algo.food,
                f"{algorithm_name}: Path should directly go to adjacent food.",
            )

    def run_test_food_on_head(
        self, algorithm_class, algorithm_name, current_mock_pygame
    ):
        configure_mock_pygame(current_mock_pygame)
        algo = algorithm_class(game_has_obstacles=False)
        ...
        self.assertEqual(
            len(algo.path),
            0,
            f"{algorithm_name}: Path should be empty if food is on head. Path: {algo.path}",
        )
    ...
    def test_dfs_simple_path(self, mock_pygame_injected):
        ...
        self.assertTrue(
            len(algo.path) > 0,
            f"DFS: Path should not be empty for a simple case. Path: {algo.path}",
        )
        if algo.path:  # Check if path is not empty before accessing its elements
            self.assertEqual(algo.path[-1], algo.food, "DFS: Last step should be food.")
        for p_idx, p_obj in enumerate(algo.path):  # Check all items are Point objects
            self.assertIsInstance(
                p_obj,
                Point,
                f"DFS: Item at index {p_idx} in path is not a Point object.",
            )
    ...
    def run_test_tail_vacate_simple(
        self, algorithm_class, algorithm_name, current_mock_pygame
    ):
        configure_mock_pygame(current_mock_pygame)
        algo = algorithm_class(game_has_obstacles=False)
        ...
        algo.snake = [algo.head, Point(BLOCK_SIZE, 0), Point(0, 0)]
        algo.food = Point(0, 0)
        algo.obstacles = []
        ...
        self.assertTrue(
            len(algo.path) > 0,
            f"{algorithm_name} TailVacateSimple: Path should be found. Path: {algo.path}",
        )
        if algo.path:
            self.assertEqual(
                algo.path[-1],
                algo.food,
                f"{algorithm_name} TailVacateSimple: Path should lead to food.",
            )
        ...
        if algorithm_name not in ["DFS"]:  # DFS path length can vary
            ...
            self.assertEqual(
                len(algo.path),
                4,
                f"{algorithm_name} TailVacateSimple: Path length should be 4. Path: {algo.path}",
            )

    def run_test_tail_vacate_with_turn(
        self, algorithm_class, algorithm_name, current_mock_pygame
    ):
        configure_mock_pygame(current_mock_pygame)
        algo = algorithm_class(game_has_obstacles=False)
        ...
        algo.head = Point(0, 0)
        algo.snake = [algo.head, Point(0, BLOCK_SIZE), Point(0, BLOCK_SIZE * 2)]
        ...
        self.assertTrue(
            len(algo.path) > 0,
            f"{algorithm_name} TailVacateTurn: Path should be found. Path: {algo.path}",
        )
        if algo.path:
            self.assertEqual(
                algo.path[-1],
                algo.food,
                f"{algorithm_name} TailVacateTurn: Path should lead to food.",
            )
        ...
        if algorithm_name not in ["DFS"]:
            ...
            self.assertEqual(
                len(algo.path),
                4,
                f"{algorithm_name} TailVacateTurn: Path length should be 4. Path: {algo.path}",
            )
