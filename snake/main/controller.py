...
from snake.configs.game import FPS, HEIGHT, WIDTH
...
class AppController:
    def __init__(self):
        ...
        self.current_game_instance = (
            None  # Renamed from game_instance for clarity with pause
        )
            ...
            if self.current_state == GameState.MAIN_MENU:
                ...
                for event in events:
                    ...
                    # Check if action is one of the known game mode actions
                        ...
                        actions.MODE_BFS,
                        actions.MODE_DFS,
                        actions.MODE_BEST_FS,
                        actions.MODE_SIMPLE_HILL_CLIMBING,
                ...
                if (
                    not self.current_game_instance
                ):  # If no active game instance (e.g., new game or after restart)
                    GameAlgorithmClass = self.get_game_class(self.selected_game_mode)
                    if GameAlgorithmClass:
                        print(
                            f"Starting/Restarting game mode: {self.selected_game_mode} with obstacles: {self.game_obstacles_enabled}"
                        )
                        self.current_game_instance = GameAlgorithmClass(
                            game_has_obstacles=self.game_obstacles_enabled
                        )
                    else:
                        print(
                            f"Error: Unknown game mode {self.selected_game_mode} or no mode selected."
                        )
                        self.current_state = GameState.MAIN_MENU
                ...
                if (
                    not self.game_over_screen
                    or self.game_over_screen.final_score != self.last_score
                ):
                    self.game_over_screen = GameOverScreen(
                        self.display, self.font, self.last_score
                    )
                ...
                for event in events:
                    ...
                    ):  # GameOverScreen uses ACTION_MAIN_MENU from actions.py
                        self.current_state = GameState.MAIN_MENU
